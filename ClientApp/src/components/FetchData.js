...
export class FetchData extends Component {
    ...
    async componentDidMount() {
        ...
        for (var j = 0; j < dataWtgs.length; j++) {
            if (dataWtgs[j].project === null && dataWtgs[j].projectId !== 0) {
                const response = await fetch('/projects/' + dataWtgs[j].projectId, {
                    headers: !token ? {} : { 'Authorization': `Bearer ${token}` }
                });
                const dataProject = await response.json();
                dataWtgs[j].project = dataProject
            }
        }
                    ...
                    <Row className="mt-5">
                        ...
                        <Col>
                            <div className="sectionHeading d-flex align-items-center mb-4">
                                ...
                                <NewWTG addData={this.addData} projects={this.state.projects} />
                            </div>
                            <div className="table-responsive">
